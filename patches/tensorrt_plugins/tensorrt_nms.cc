...
nvinfer1::IPluginV2DynamicExt *NonMaxSuppressionDynamic::clone() const noexcept {
...
int NonMaxSuppressionDynamic::enqueue(
  ...
  TRTNMSCUDAKernelLauncher_float(
      ...
      0 /*offset*/, output, mCenterPointBox, num_batches, spatial_dimension,
...
// IPluginV2 Methods
const char *NonMaxSuppressionDynamic::getPluginType() const noexcept {
  return PLUGIN_NAME;
}

const char *NonMaxSuppressionDynamic::getPluginVersion() const noexcept {
  return PLUGIN_VERSION;
}

int NonMaxSuppressionDynamic::getNbOutputs() const noexcept { return 1; }

int NonMaxSuppressionDynamic::initialize() noexcept { return 0; }

void NonMaxSuppressionDynamic::terminate() noexcept {}

size_t NonMaxSuppressionDynamic::getSerializationSize() const noexcept {
  return sizeof(mCenterPointBox) + sizeof(mMaxOutputBoxesPerClass) +
         sizeof(mIouThreshold) + sizeof(mScoreThreshold) + sizeof(int /*offset*/);
}

void NonMaxSuppressionDynamic::serialize(void *buffer) const noexcept {
...
void NonMaxSuppressionDynamic::destroy() noexcept {
  // This gets called when the network containing plugin is destroyed
  delete this;
}

void NonMaxSuppressionDynamic::setPluginNamespace(const char *libNamespace) noexcept {
  mNamespace = libNamespace;
}

const char *NonMaxSuppressionDynamic::getPluginNamespace() const noexcept {
...
NonMaxSuppressionDynamicCreator::NonMaxSuppressionDynamicCreator() {
  ...
  mPluginAttributes.emplace_back(nvinfer1::PluginField("center_point_box", nullptr, nvinfer1::PluginFieldType::kINT32));
  mPluginAttributes.emplace_back(nvinfer1::PluginField("max_output_boxes_per_class", nullptr, nvinfer1::PluginFieldType::kINT32));
  mPluginAttributes.emplace_back(nvinfer1::PluginField("iou_threshold", nullptr, nvinfer1::PluginFieldType::kFLOAT32));
  mPluginAttributes.emplace_back(nvinfer1::PluginField("score_threshold", nullptr, nvinfer1::PluginFieldType::kFLOAT32));
  mFC.nbFields = mPluginAttributes.size();
  mFC.fields = mPluginAttributes.data();
}

const char *NonMaxSuppressionDynamicCreator::getPluginName() const noexcept {
  return PLUGIN_NAME;
}

const char *NonMaxSuppressionDynamicCreator::getPluginVersion() const noexcept {
  return PLUGIN_VERSION;
}

const nvinfer1::PluginFieldCollection *
NonMaxSuppressionDynamicCreator::getFieldNames() noexcept {
  return &mFC;
}
...
const char *NonMaxSuppressionDynamicCreator::getPluginNamespace() const noexcept {
  return mNamespace.c_str();
}
